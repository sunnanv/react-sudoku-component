...
*/
  ...
  */
  ...
  /*  static generateSudoku(difficulty) {
        ...
        let solvedSudoku;
        do {
            let firstRow = this.generateSudokuRow();            
            let lastRow = this.generateSudokuRow([firstRow]);
            ...
            const wait = new Promise((resolve) => setTimeout(resolve, 5000, false));
            solved = Promise.race([this.solve(solvedSudoku), wait]);
            
        } while(solved !== true);

        let sudoku = this.createSudokuFromSolvedSudoku(solvedSudoku, difficulty);
     ...
     */
  /*  static generateSudokuRow(matchingRows = []) {
     ...
     */
 /*   static createSudokuFromSolvedSudoku(solvedSudoku, difficulty) {

        const getRandomCellNumber = () => Math.floor(Math.random()*81);
        let nbrsToAdd = this.difficulties[difficulty];
